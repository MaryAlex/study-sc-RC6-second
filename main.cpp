...
int main() {
    ...
    long N = 1;
    int n = 128;
    std::string *vectors = getNVectors(N, n, true, "0", false);
    std::string key = getVector(keyLength * bitsInByte, false, "0", false);
    std::string *encryptResult = encryptBlocks(rc6, vectors, N, key);
    std::string decryptResult = rc6->run(RC6_DECRYPT_MODE, stringToHex(encryptResult[0]), stringToHex(key));
    writeInFile("result1.bin", encryptResult, N);
    std::cout << encryptResult[0] << std::endl;
